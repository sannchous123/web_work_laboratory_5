...
<body>
    ...
    <form action="form.php" method="POST">
        <p class="<?= isset($cookie_errors['full_name']) ? 'has-error' : '' ?>">
            <label>ФИО:<br>
                <input type="text" name="full_name" value="<?= htmlspecialchars($merged_old['full_name'] ?? '') ?>" required>
            </label>
            <?php if (isset($cookie_errors['full_name'])): ?>
                <br><span class="error"><?= htmlspecialchars($cookie_errors['full_name']) ?></span>
            <?php endif; ?>
        ...
        <p class="<?= isset($cookie_errors['phone']) ? 'has-error' : '' ?>">
            <label>Телефон:<br>
                <input type="tel" name="phone" value="<?= htmlspecialchars($merged_old['phone'] ?? '') ?>" required>
            </label>
            <?php if (isset($cookie_errors['phone'])): ?>
                <br><span class="error"><?= htmlspecialchars($cookie_errors['phone']) ?></span>
            <?php endif; ?>
        ...
        <p class="<?= isset($cookie_errors['email']) ? 'has-error' : '' ?>">
            <label>E-mail:<br>
                <input type="email" name="email" value="<?= htmlspecialchars($merged_old['email'] ?? '') ?>" required>
            </label>
            <?php if (isset($cookie_errors['email'])): ?>
                <br><span class="error"><?= htmlspecialchars($cookie_errors['email']) ?></span>
            <?php endif; ?>
        ...
        <p class="<?= isset($cookie_errors['birth_date']) ? 'has-error' : '' ?>">
            <label>Дата рождения:<br>
                <input type="date" name="birth_date" value="<?= htmlspecialchars($merged_old['birth_date'] ?? '') ?>" required>
            </label>
            <?php if (isset($cookie_errors['birth_date'])): ?>
                <br><span class="error"><?= htmlspecialchars($cookie_errors['birth_date']) ?></span>
            <?php endif; ?>
        ...
        <p class="<?= isset($cookie_errors['gender']) ? 'has-error' : '' ?>">
            ...
            <?php if (isset($cookie_errors['gender'])): ?>
                <br><span class="error"><?= htmlspecialchars($cookie_errors['gender']) ?></span>
            <?php endif; ?>
        </p>
            ...
            <label>Любимые языки программирования:<br>
                <select name="languages[]" multiple size="6" required>
                    <?php foreach ($languages as $lang): ?>
                        ...
                        <option value="<?= $lang['id'] ?>" <?= $selected ? 'selected' : '' ?>><?= htmlspecialchars($lang['language_name']) ?></option>
                    <?php endforeach; ?>
                </select>
            </label>
            <?php if (isset($cookie_errors['languages'])): ?>
                <br><span class="error"><?= htmlspecialchars($cookie_errors['languages']) ?></span>
            <?php endif; ?>
        </p>

        <p class="<?= isset($cookie_errors['biography']) ? 'has-error' : '' ?>">
            <label>Биография:<br>
                <textarea name="biography" rows="4"><?= htmlspecialchars($merged_old['biography'] ?? '') ?></textarea>
            </label>
            <?php if (isset($cookie_errors['biography'])): ?>
                <br><span class="error"><?= htmlspecialchars($cookie_errors['biography']) ?></span>
            <?php endif; ?>
        ...
        <p class="<?= isset($cookie_errors['agreed_to_contract']) ? 'has-error' : '' ?>">
            ...
            <?php if (isset($cookie_errors['agreed_to_contract'])): ?>
                <br><span class="error"><?= htmlspecialchars($cookie_errors['agreed_to_contract']) ?></span>
            <?php endif; ?>
        </p>
