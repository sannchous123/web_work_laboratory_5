...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['username']) && isset($_POST['password']) && !isset($_POST['update'])) {
    ...
    try {
        ...
        $_SESSION['login_error'] = 'Ошибка БД.';
        header('Location: login.php');
        exit();
    }
}
...
<body>
    ...
    <?php else: ?>
        <p>Вы вошли как: <strong><?= htmlspecialchars($_SESSION['username']) ?></strong> | <a href="?logout=1">Выйти</a></p>
        ...
        <form method="POST">
            <p><label>ФИО: <input type="text" name="full_name" value="<?= htmlspecialchars($user_data['full_name'] ?? '') ?>" required></label></p>
            <p><label>Телефон: <input type="tel" name="phone" value="<?= htmlspecialchars($user_data['phone'] ?? '') ?>" required></label></p>
            <p><label>E-mail: <input type="email" name="email" value="<?= htmlspecialchars($user_data['email'] ?? '') ?>" required></label></p>
            <p><label>Дата рождения: <input type="date" name="birth_date" value="<?= htmlspecialchars($user_data['birth_date'] ?? '') ?>" required></label></p>
            <p>
                Пол:
            ...
            <p><label>Биография:<br><textarea name="biography" rows="4"><?= htmlspecialchars($user_data['biography'] ?? '') ?></textarea></label></p>
